...
def set_protected_branch(token, repo, branch):
    ...
    gql_get_repo_id = gql(
        ...
        query get_repo_id($repo: String!) {
            repository(name: $repo, owner: "synopkg") {
                id
            }
        }
        """
...
def main():
    ...
    org = gh.get_organization("synopkg")
    ...
    synopkg = org.get_repo("synopkg")
    pr_id = int(github_event["issue"]["number"])
    pr = synopkg.get_pull(pr_id)
    ...
    print("Creating new repo on Synopkg")
    repo = org.create_repo(appid)
    time.sleep(5)
    repo.edit(homepage=f"https://synopkg.org/apps/details/{appid}")

    print("Adding synopkg remote")
    clone.remotes.create(
        "synopkg", f"https://x-access-token:{github_token}@github.com/synopkg/{appid}"
    )
    ...
    print("Pushing changes to the new Synopkg repo")
    git_push = f"cd {tmpdir.name} && git push synopkg {branch}:{remote_branch}"
    ret = subprocess.run(
        git_push,
        shell=True,
        check=True,
    ...
    print(ret.stdout)
    print(ret.stderr)
    repo.remove_from_collaborators("synopkgbot")
    ...
    close_comment = (
        ...
        "\n",
        "If you have never maintained an application before, common questions are answered in [the app maintenance guide](https://github.com/synopkg/synopkg/wiki/App-Maintenance).",
        "\n",
        "Thanks!",
    )
